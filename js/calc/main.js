...
for (const country in allPriceJson) {
    ...
    input.id = "place" + (listCountry.querySelectorAll("div.cheakbox__item__btn").length + 1);
    div.append(input);
...
clacBtn.addEventListener("click", calcCheck);
...
function calcCheck() {
    ...
    document.querySelectorAll(".city input").forEach((item) => {
        cityChecked = cityChecked || item.checked;
        
    })
    let durationsChecked = false;
    durations.forEach((duration) => {
        durationsChecked = durationsChecked || duration.parentElement.querySelector("input").checked
    })
    if (!(cityChecked && durationsChecked)) {
        calcError.classList.remove("none")
    } else {
        calcError.classList.add("none")
    }
    // console.log(!(cityChecked && durationsChecked))
}
